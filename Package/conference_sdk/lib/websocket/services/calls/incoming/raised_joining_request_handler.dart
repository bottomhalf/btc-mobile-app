...
class RaisedJoiningRequestHandler {
  void execute(ServerEventService service, CallIncomingEvent event) {
    ...
    if (currentUser != null && event.callerId == currentUser.userId) {
      debugPrint('[ServerEvents] Ignoring raised joining request — I am the caller');
      return;
    }

    service.store.setJoiningRequest(event, CallStatus.raisedJoiningRequest);
    debugPrint('[ServerEvents] Raised joining request from: ${event.callerId}');
  }
}
